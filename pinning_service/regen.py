...
def generate_anchor_tx(
    sender: Address, resolver_id: int, content_hashes: Iterable[ContentHash]
) -> dict:
...
def anchor(content_hashes: Iterable[ContentHash]) -> TxHash:
    ...
    tx = generate_anchor_tx(
        settings.REGEN_KEY_ADDRESS, settings.REGEN_RESOLVER_ID, content_hashes
    )
    ...
    execute_command = f"echo '{json.dumps(tx)}' | {settings.REGEN_CLI_COMMAND} regen tx authz exec /dev/stdin --from {settings.REGEN_SERVICE_KEY_ADDRESS} --fee-account {settings.REGEN_KEY_ADDRESS} {chain_id} {node} {output} {settings.REGEN_KEYRING_ARGS}"
    try: 
